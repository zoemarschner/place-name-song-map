...
new_csv_data = [['Song', 'Artist', 'Image', 'Lat', 'Lon']]
...
def get_songs():
	...
	with open('songs.csv') as csvfile:
		...
		for row in song_reader:
			...
			new_csv_row.append(row[0])
			new_csv_row.append(row[1])
			...
			new_csv_row.append(img_url)


			# now get location data!!!
			loacation_search_term = row[2]
			response = requests.get(f"https://maps.googleapis.com/maps/api/geocode/json?address={loacation_search_term.replace(' ', '+')}&key={google_api_key}")
			place_data = response.json()['results']
			if len(place_data) < 1:
				print(f"COULDN'T FIND {loacation_search_term}")
				continue
			lat_lon = place_data[0]['geometry']['location']

			new_csv_row += [lat_lon['lat'], lat_lon['lng']]

			new_csv_data.append(new_csv_row)


	with open("song_data.csv", "w+") as new_csv:
